...
class IRSystem:
    ...
    def compute_tfidf(self):
        ...
        doc_sets = collections.defaultdict(lambda: set())
        num_docs = len(self.docs)
        tf = collections.defaultdict(lambda: collections.defaultdict(lambda: 0))

        # calculate term-doc counts
        for doc_idx, doc in enumerate(self.docs):
            for word in doc:
                doc_sets[word].add(doc_idx)
                tf[word][doc_idx] += 1

        # take logs of term-doc counts, calc cum by doc
        for word in tf:
            for doc_idx in tf[word]:
                log_value = 1 + np.log10(tf[word][doc_idx])
                tf[word][doc_idx] = log_value

        # calculate idf by word
        idf = dict()
        for word in doc_sets:
            idf[word] = np.log10(float(num_docs) / len(doc_sets[word]))

        # build sparse tf-idf matrix
        self.tfidf = {}
        sq_sums_by_doc = collections.defaultdict(lambda: 0.0)
        for word in self.vocab:
            for d in range(len(self.docs)):
                if word not in self.tfidf:
                    self.tfidf[word] = {}
                self.tfidf[word][d] = tf[word][d] * idf[word]
                sq_sums_by_doc[d] += self.tfidf[word][d] ** 2

        # build sparse normalized tf-idf matrix, with unit-length document vectors
        self.tfidf_normalized = {}
        for word in self.vocab:
            for d in range(len(self.docs)):
                if word not in self.tfidf_normalized:
                    self.tfidf_normalized[word] = {}
                self.tfidf_normalized[word][d] = self.tfidf[word][d] / np.sqrt(sq_sums_by_doc[d])
    ...
    def index(self):
        ...
        inv_index = collections.defaultdict(lambda: list())
    ...
    def boolean_retrieve(self, query):
        ...
        for word in query:
            docs_matching_word = set(self.get_posting(word))
            docs = docs.intersection(docs_matching_word)
    ...
    def rank_retrieve(self, query):
        ...
        query_counts = collections.defaultdict(lambda: 0)
        for word in query:
            query_counts[word] += 1
        tf_query = dict()
        for word in query_counts:
            tf_query[word] = 1 + np.log10(query_counts[word])

        for word in tf_query:
            docs_with_word = self.tfidf_normalized[word]
            for doc_idx in docs_with_word:
                scores[doc_idx] += docs_with_word[doc_idx] * tf_query[word]
